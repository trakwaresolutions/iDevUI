...
//
// This section is courtesy of John Resig @ www.ejohn.org
///////////////////////////////////////////////////////////////////////////////
(function()
{
...
var idevResize = idevObject.extend({
		...
		syncSize: function()
		{
			...
			$('#' + this.widget.id + "-drhandlese").css({ 'left':(w-2), 'top':(h-2) });
			$('#' + this.widget.id + "-drhandles").css({ 'top':(h-2), 'left':(w/2) });
...
var idevLayoutManager = idevObject.extend({
		...
		resizePanel: function()
		{
			...
			$("#" + this.id + "-background").css({ 'width':w, 'max-width':w });
			$("#" + this.id + "-body").css({ 'width':w, 'max-width':w });
			$("#" + this.id + "-content").css({ 'width':w, 'max-width':w });
			...
			{
				$("#" + this.id + "-title").css({ 'width':w, 'max-width':w });
			}
			if (this.widget.tbar)
			{
				$("#" + this.id + "-tbar").css({ 'width':w, 'max-width':w });
				this.widget.tbar.doLayout();
			}
			if (this.widget.bbar)
			{
				$("#" + this.id + "-bbar").css({ 'width':w, 'max-width':w });
			...
			try
			{
				...
				else if (this.layout == "frame")
				{
					...
					for (var c = 0;c < this.widget.children.length;c++)
					{
						...
						if (this.widget.children[c].area == "south")
						{
							$("#"+id).css({ 'width':pw, 'max-width':pw });
							$("#"+id+"-body").css({ 'width':pw, 'max-width':pw });
							$("#"+id+"-background").css({ 'width':pw, 'max-width':pw });
						}
						...
						{
							$("#"+id).css({ 'height':hcenter, 'max-height':hcenter });
							$("#"+id+"-body").css({ 'height':hcenter, 'max-height':hcenter });
							$("#"+id+"-background").css({ 'height':hcenter, 'max-height':hcenter });
							this.widget.children[c].doLayout();							
						}
						else if (this.widget.children[c].area == "center")
						{
							$("#"+id).css({ 'height':hcenter, 'max-height':hcenter, 'width':wcenter, 'max-width':wcenter });
							$("#"+id+"-body").css({ 'height':hcenter, 'max-height':hcenter, 'width':wcenter, 'max-width':wcenter });
							$("#"+id+"-background").css({ 'height':hcenter, 'max-height':hcenter, 'width':wcenter, 'max-width':wcenter });
...
var baseWidget = idevObject.extend(
	{
		init: function(config)
		{
			...
			if(this.parent && (this.parent.id == "ui-toolbar" || (this.parent.parent && this.parent.parent.id == "ui-toolbar")))
			{
			...
			this.resizing = false;
			this.inLayout = false;
			this.tabindex = this.config.tabindex || 0;
		...
		{					 
			...
			this.resizing = true;
			var elem = $("#" + this.id);
			elem.css({ 'width':w, 'max-width':w, 'height':h, 'max-height':h });
...
var idevCore = idevObject.extend({
	...
	internal:
	{
				...
				if (idev.isIPad()|| idev.isIPhone() || idev.isAndroid())
				{
					if (_preferences.orientation == "portrait" || _preferences.orientation == "landscape")
					{
						if (_preferences.orientation == "landscape")
							$("body").prop("orient","landscape");
						else
							$("body").prop("orient","profile");
					}
					else
					{
		...
		bind: function()  //This function binds the mouse and touch events
		{
			...
			$('#container').bind("mouseleave", function(e){
				if(idev._dragging)
				{
					...
					var target = idev._dragWidget;
					target.moveTo((idev.pgWidth/2)-(target.width/2), (idev.pgHeight/2)-(target.height/2));
				}
			});
			...
			switch(wgt.wtype)
			{
				...
				default:
					...
					if (wgt.enabled)
					{
						...
						{
							idev.errorHandler("onClick Error: "+e.message);
						}
					}
					e.preventDefault();
					e.stopPropagation();
		...
		onFocus: function(e)
		{
			...
			if (wgt)
			{
				if (wgt.watermark != "")
				{
					...
					{
						$('#'+wgt.id+"-input").val("").css("color","");
						if(wgt.inputType == "password" && !wgt.showPassword){
							$delay(0,function(){
								$('#'+wgt.id+"-input").prop('type', 'password');
							});
						}
					}
				}
					...
					if (widget.wtype != null)
					{
						...
						if (parent.config)
						{
							if (parent.config.center)
								sHTML += "<td width='"+parent.width+"' style='" + cellStyle + " margin: 0 auto;'><div id='" + renderTo + "'></div></td>";
							else
							{
								if (parent.layoutConfig)
								{				   
		...
		limiter : function(elem, limit)
		{
			if(!elem || !limit) return;
			if(!this.limiterIncluded)
			{
				...
				$.fn.extend( {
					limiter: function(limit, elem) {
						...
						function setCount(src, elem) {
							...
							if(elem) elem.html( limit - chars );
						}
						setCount($(this)[0], elem);
					}
				});
				this.limiterIncluded = true;
			}
			elem.limiter(limit);
		}
	},
	...
	*/
		...
		dataStore : idevObject.extend(
		{
			...
			updateBinds : function(fromload)
			{
				...
				for (var i = 0;i < this.binds.length;i++)
				{
					...
					{
						idev.errorHandler("Update bind failed: "+this.binds[i].id);
					}
				}
	...
	page : idevObject.extend({
		...
		render : function()
		{
			...
			sHTML += "<div id='" + this.config.id + "' class='ui-page " + this.config.cls + "' style='width:" + cw + "px;height:" + ch + "px;" + this.config.pagestyle + "'>";
	...
	pageManagerClass : idevObject.extend({
		...
		createWidgets: function(config, newpage)
		{
			...
			if (config.content.widgets != null)
			{
				...
				var layout = config.content.layout || config.layout || "flow";
				idev.internal.renderWidgets(newpage, newpage.config.id + "-content", config.content.widgets, layout,  config.content.labelWidth, config.content)
			}
		...
		showPage : function (index)
		{
			...
			{
				if (idev.pageManager.currentPage.events.beforeHide) idev.pageManager.currentPage.events.beforeHide(idev.pageManager.currentPage);
			}
			...
			if (idev.pageManager.newPage && pg.config.hidden)
			{
				...
				{
					elem.css({ 'visibility':'visible', 'left':0-(idev.pgWidth+10) });
				}
				else
				{
					elem.css({ 'z-index':-10, 'visibility':'visible' });
				}
				return true;
			}
			if (idev.pageManager.animation)
			{
				if (idev.pageManager.animationType == "fade")
				{
					...
					{
						$("#" + p.id).css({ 'visibility':'hidden', 'display':'block' });
					...
					idev.pageManager.currentPage = pg;
					elem.css({ 'display':'none', 'visibility':'visible'  });
					elem.fadeIn(idev.fadeInTime, function()
					{
				...
				if (idev.pageManager.animationType == "slide")
				{
					...
					else
					{
						...
						elem.css({ 'z-index':2, 'visibility':'visible' });
						idev.utils.delay(100,function()
						{	   
							elem.animate({
									left:0
								...
								{
									var x = 0-(idev.pgWidth+10);
									$('#' + idev.pageManager.currentPage.id).css({ 'left':x, 'visibility':'hidden' });
									idev.pageManager.activeIndex = idev.pageManager.pageIndex;
									idev.pageManager.currentPage = pg;
									idev.pageManager.canNavigate = true;
									if (pg.events != null)
			...
			else
			{
				...
				{
					$("#" + this.currentPage.id).css({ 'visibility':'hidden', 'z-index':-10 });
					this.currentPage = pg;
					this.activeIndex = this.pageIndex;
					elem.css({ 'z-index':0, 'visibility':'visible' });
					if (idev.pageManager.newPage)
					{
						idev.pageManager.createWidgets(idev.app.pages[idev.pageManager.pageIndex], pg);
					}
		...
		{	  
			...
			if (idev.app.events && idev.app.events.onResize) idev.app.events.onResize();
		}
	...
	utils :
	{
		ajax : function( config )
		{
			...
			{	
				...
				if(idev.app && idev.app.token) config.params.token = idev.app.token;
		...
		dateAdd : function (objDate, strInterval, intIncrement)
		{
			if(typeof(objDate) == "string")
			{
				...
				if (isNaN(objDate))
				{
					idev.errorHandler("DateAdd: Date is not a valid date");
				}
			}
			else if(typeof(objDate) != "object" || objDate.constructor.toString().indexOf("Date()") == -1)
			{
				idev.errorHandler("DateAdd: First parameter must be a date object");
			}
			...
			{
				idev.errorHandler("DateAdd: Second parameter must be M, D, Y, h, m, uM, uD, uY, uh, um or us");
			}
			if
			(typeof(intIncrement) != "number")
			{
				idev.errorHandler("DateAdd: Third parameter must be a number");
			}
		...
		{
			for(var key in idev.widgets)
			{
				var widget = idev.widgets[key];
				if(widget.wtype == "window")
					widget.close();
			}
		},
	...
	ui :
	{
				...
				bbar = [
				   '>>',
					...
					},
					{
						...
						radius:4,
						text:'CANCEL',
						cls:'ui-button ui-button-cancel',
						events : {
							click: function(btn)
							{
		...
		widgetPanel : baseWidget.extend(
		{
			...
			addWidget: function(wgt)
			{
				...
				else if (this.layout == "row")
				{
					...
					if (this.center)
						sHTML += "<td style='" + cellStyle + " margin: 0 auto;'><div id='" + renderTo + "'></div></td>";
					else
						sHTML += "<td id='" + renderTo + "' style='" + cellStyle + "'></td>";
					sHTML += "</tr>";
					$('#' + tableID).append(sHTML);
		...
		widgetTextField : baseWidget.extend(
		{
			init: function(config)
			{
				...
				this.tpl = new idev.wTemplate(
						...
						"<input id='{id}-input' type='{inputtype}' maxlength='{maxlength}' class='ui-input {inputcls}' style='width:{width}px;height:{height}px;max-width:{width}px;max-height:{height}px;{inputstyle}'  value='{value}' title='{title}' tabindex='{tabindex}' />",
						"</div>"
					);
			...
			togglePassword: function(set)
			{
				if(idev.isIE8())
				{
					$debug('togglePassword() is unsupported in IE8.');
				}
				else
				{
					if(this.inputType != "password") return;
					var field = $('#'+this.id+'-input');
					if(field.prop('type') == "password" || set === false)
					{
						this.showPassword = true;
						if((field[0].value != this.watermark))
							field.prop('type','text');
					}
					else if(field.prop('type') == "text" || set === true)
					{
			...
			render : function()
			{
				...
				if (this.roundCorners)
				{
					$("#" + this.id).css({ 'border-radius':this.radius+'px', '-webkit-border-radius':this.radius+'px' });
				}
				...
				if(this.watermark != "" && this.value == "") $('#'+this.id+'-input').prop('type', 'text');
		...
		widgetTextArea : baseWidget.extend(
		{
			init: function(config)
			{
				...
				this.tpl = new idev.wTemplate(
						...
						"<textarea id='{id}-input' class='ui-textarea {inputcls}' maxlength='{maxlength}'  style='width:{width}px;height:{height}px;{inputstyle};' title='{title}' tabindex='{tabindex}'>{value}</textarea>",
						"</div>"
					);
				if (this.inputCls != "") this.inputCls += "  ie9gradient";
			},
		...
		widgetComboBox : baseWidget.extend(
		{
			init: function(config)
			{
				...
				this.tpl = new idev.wTemplate(
						...
						"<input id='{id}-input' type='{inputtype}' class='ui-combo-input' style='width:{width}px;height:{inputheight}px;{inputstyle}' value='{value}' title='{title}' tabindex='{tabindex}'/>",
			...
			init: function(config)
			{
				...
				if (this.align == "right")
					this.tpl = new idev.wTemplate(
							...
							"<tr>",
							"<td width='20px'>",
							"<button id='{id}-input' class='ui-checkbox {cls}' tabindex='{tabindex}'>&nbsp;&nbsp;&nbsp;&nbsp;",
							"</button>",
							"</td>",
							"<td id='{id}-text'>",
		...
		widgetSwitch : baseWidget.extend(
		{
			...
			setOn : function(switchon)
			{
				if (switchon)
				{
				   $('#'+this.id+"-img").prop('src', _preferences.libpath + '/images/on.png');
				   this.on = true;
				}
				else
				{
				   $('#'+this.id+"-img").prop('src', _preferences.libpath + '/images/off.png');
				   this.on = false;
				}
			},
			getValue:function()
		...
		widgetRadio : baseWidget.extend(
		{
			init: function(config)
			{
				...
				if (this.align == "right")
					this.tpl = new idev.wTemplate(
							...
							"<tr>",
							"<td width='20px'>",
							"<div id='{id}-input' group='{group}' class='ui-radio-off {cls}' tabindex='{tabindex}'>&nbsp;&nbsp;&nbsp;&nbsp;",
							"</div>",
							"</td>",
							"<td id='{id}-text'>",
			...
			getValue : function()
			{
				var id = $('[group='+this.group+'].ui-radio-on').prop('id').replace('-input','');
		...
		widgetSVGButton : baseWidget.extend(
		{
			init: function(config)
			{
				...
				this.tpl = new idev.wTemplate("<div id='{id}' class='ui-element {buttoncls}' style='{elementstyle}{style}' title='{title}' tabindex='{tabindex}'></div>");
		...
		widgetButton : baseWidget.extend(
		{
			init: function(config)
			{
				...
				{
					this.tpl = new idev.wTemplate(
						"<button unselectable='on' type='button' id='{id}' class='ui-element {cls} {togglecls} unselectable' style='{elementstyle}{style}' title='{tooltip}' tabindex='{tabindex}'>",
						"{image1}{image2}",
						"</button>"
					);
				}
		...
		widgetImage : baseWidget.extend(
		{
			...
			setSrc :function(sSrc,width,height)
			{
				...
				height = height || "";
				if (width == "" && height == "")
					$( '#' +  this.id + "-image" ).prop( { src:sSrc.toUpperCase() == "BLANK" || sSrc == "" ? _preferences.blankimage : sSrc });
				else
				{
		...
		widgetProgress : baseWidget.extend(
		{
			...
			render : function()
			{
				...
				{
					$("#" + this.id).css({ 'border-radius':this.radius+'px', '-webkit-border-radius':this.radius+'px' });
					$("#" + this.id + "-bar").css({ 'border-radius':this.radius+'px', '-webkit-border-radius':this.radius+'px' });
					$("#" + this.id + "-progress").css({ 'border-radius':this.radius+'px', '-webkit-border-radius':this.radius+'px' });
				}
				idev.internal.afterRender(this);
				if (this.events.click)
				{
		...
		widgetObject : baseWidget.extend(
		{
			init: function(config)
			{
				...
				this.fallback = config.fallback == null ? "" : config.fallback;
				this.tpl = new idev.wTemplate(
					"<div id='{id}' class='ui-element' style='{elementstyle};{style};'>",
					"<object id='{id}-object' type='{type}' data='{src}' style='height:{height}px;width:{width}px;overflow:auto;'>{fallback}</object>",
					"</div>" );
		...
		widgetGrid : baseWidget.extend(
		{
			...
			build : function()
			{
				...
				if (this.showHeader)
				{
					...
					for (var c = 0,count = this.cm.getCount();c < count;c++)
					{
						...
						if(c == 0)
						{
							sHTML += "<td id='"+id+"' width=" + (w-1) + " height=" + this.headerHeight + " class='ui-grid-header'><div style='float:left;line-height:"+this.headerHeight+"px;width:" + (w-1) +"px;text-align:center;'>" + sHeader + "</div></td>";
						}
						else if(c == this.cm.getCount()-1)
						{
							if(idev.isIE8())
								...
								w += 25;
							sHTML += "<td id='"+id+"' width=" + w + " height=" + this.headerHeight + " class='ui-grid-header'><div class='ui-grid-separator' style='height:" + this.headerHeight + "px;'></div><div style='float:none;line-height:"+this.headerHeight+"px;width:" + (col.width) +"px;text-align:center;'>" + sHeader + "</div></td>";
						}
						else
							sHTML += "<td id='"+id+"' width=" + w + " height=" + this.headerHeight + " class='ui-grid-header'><div class='ui-grid-separator' style='height:" + this.headerHeight + "px;'></div><div style='float:none;line-height:"+this.headerHeight+"px;width:" + (col.width) +"px;text-align:center;'>" + sHeader + "</div></td>";
					}
			...
			render : function()
			{
				...
				{
					$("#" + this.id).css({ 'border-radius':this.radius+'px', '-webkit-border-radius':this.radius+'px' });
					$("#" + this.id+"-body").css({ 'border-radius':this.radius+'px', '-webkit-border-radius':this.radius+'px' });
					var r = this.radius-1;
					if (this.tbar)
					{
						$("#" + this.id + "-tbar").css({
							'border-radius-topleft'          :r+'px', 
							'-webkit-border-radius-topleft'  :r+'px',
							'border-radius-topright'         :r+'px',
							'-webkit-border-radius-topright' :r+'px'
						});
					}
					else
					{
					...
					{
						$("#" + this.id).css({
							'border-radius-bottomleft'          :r+'px', 
							'-webkit-border-radius-bottomleft'  :r+'px',
							'border-radius-bottomright'         :r+'px',
							'-webkit-border-radius-bottomright' :r+'px'
						});
					}
				}
			},
		...
		widgetTabPanel : baseWidget.extend(
		{
			...
			render : function()
			{
				...
				{
					$("#" + this.id).css({
						'border-bottom-left-radius':          this.radius+'px', 
						'-webkit-border-bottom-left-radius':  this.radius+'px',
						'border-bottom-right-radius':         this.radius+'px',
						'-webkit-border-bottom-right-radius': this.radius+'px'
					});
					var r = this.radius;
					if (this.border) r--;
					$("#" + this.id + "-panels").css({
			...
			addTab: function(tabConfig)
			{
				...
				o.title = "<div class='ui-tab' style='height:"+(this.tabHeight-2)+"px;'><div style='position:absolute;left:1px;width:" + (this.tabWidth-2) + "px;text-align:center;'><span id='"+this.id+"-tab-text" + tab + "'>"+o.tabTitle+"</span></div><div id='" + this.id + "-tabclose" + tab + "' class='ui-tab-btn-close' style='display:none;'></div></div>";
		...
		widgetComposite : baseWidget.extend(
		{
			...
			doLayout: function()
			{
				this._super();
				var w = $("#" + this.id).width();
				if(this.wtype=='toolbar'){
					w = $("#" + this.parent.id).width();
				}
	 ...
	 //			}
			...
			render : function()
			{
				...
				if (this.roundCorners)
				{
					...
					var r = this.radius;
					if (this.border) r--;
					$("#" + this.id+"-background").css({ 'border-radius': r+'px', '-webkit-border-radius': r+'px' });
			  }
				this.rendered = true;
				this.refresh();
			},
		...
		widgetDataView : baseWidget.extend(
		{
				...
				if (this.columns == "auto")
				{
					...
					for (var i = this.offset, count = this.ds.getCount();i < count && i < this.offset + this.limit;i++)
					{
						...
						if (bAdd)
						{
							...
							var entryWidth = "auto";
							var entryHeight = "auto";
							if(typeof this.entryWidth != "undefined") entryWidth = this.entryWidth + "px";
							if(typeof this.entryHeight != "undefined") entryHeight = this.entryHeight + "px";
							sHTML += "<div id='" + this.id + "_" + i + "' class='ui-dataview-wrapper "+this.entryCls+"' style='float:left;"+cellstyle+";width:"+entryWidth+";height:"+entryHeight+";max-width:"+entryWidth+";max-height:"+entryHeight+";'>" + sEntry + "</div>";
						}
					}
					return sHTML;
				}
			...
			render : function()
			{
				...
				if (this.roundCorners)
				{
					...
					var r = this.radius;
					if (this.border) r--;
					$("#" + this.id+"-wrapper").css({ 'border-radius': r+'px', '-webkit-border-radius': r+'px' });
				}
				// if (this.autoScroll && idev.isTouch())
				// {
